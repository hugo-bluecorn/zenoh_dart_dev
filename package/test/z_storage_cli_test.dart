...
void main() {
  ...
  group('z_storage CLI', () {
    ...
    test('put then query returns stored value', () async {
      ...
      expect(getStdout, contains('value1'),
          reason:
              'z_get should return stored value. stdout: $getStdout, stderr: ${getResult.stderr}');
    }, timeout: Timeout(Duration(seconds: 60)));

    test('delete removes from storage then query omits deleted key', () async {
    ...
    test('query with non-matching key returns no results', () async {
      ...
      final getStdout = getResult.stdout as String;
      expect(getStdout, isNot(contains('testval')),
          reason:
